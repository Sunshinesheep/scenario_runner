...
class WaypointFollower2(AtomicBehavior):

    """
    This is an atomic behavior to follow waypoints while dynamically adjust its plan.
    If no plan is provided, the actor will follow its foward waypoints indefinetely.
    Otherwise, the behavior will end with SUCCESS upon reaching the end condition of the plan.
    ...
    def __init__(self, actor, ref_car_name, rel_pos: int , lane_opt=None, target_speed=20, plan=None, avoid_collision=False,
                 name="FollowWaypoints2"):
        """
        Set up actor and local planner
        """
        super().__init__(name, actor)
        self._ref_actor = CarlaDataProvider.get_actor_by_name(ref_car_name)
        self._rel_pos = rel_pos
        self._lane_opt = lane_opt
        self._actor = actor
        self._target_speed = target_speed
        self._local_planner: Optional[LocalPlanner] = None
        self._plan = plan
    ...
    def initialise(self):
        super(WaypointFollower2, self).initialise()
    ...
    def _apply_local_planner(self, actor):
        ...
        if isinstance(actor, carla.Walker):
            if self._plan is not None:
                if not isinstance(self._plan[0], carla.Location):
                    self._plan = [element[0].transform.location for element in self._plan]
        else:
    ...
    def _update_local_planner(self):
        assert self._local_planner
    ...
    @staticmethod
    def _get_signed_distance(transform_a, transform_b):
        """
        Calculate the signed distance from transform_b to transform_a along transform_a's forward direction.

        Args:
        - transform_a (carla.Transform): The reference transform.
        - transform_b (carla.Transform): The target transform.

        Returns:
        - float: Positive if transform_b is ahead, negative if behind, 0 if aligned.
        """
        # Get the forward direction of transform_a
        forward_vector = transform_a.get_forward_vector()

        # Compute the relative vector from transform_a to transform_b
        relative_vector = transform_b.location - transform_a.location

        # Project relative vector onto the forward vector to get signed distance
        signed_distance = (
                relative_vector.x * forward_vector.x +
                relative_vector.y * forward_vector.y
        )

        return signed_distance

    def _get_relative_distance(self, ref_car, npc):
        """
        Check if the NPC is ahead or behind the reference car within a given distance.

        Args:
        - npc (carla.Actor): The NPC actor.
        - ref_car (carla.Actor): The reference car (ego).
        - distance (float): Distance threshold in meters.

        Returns:
        - float: Positive if NPC is ahead, negative if behind, 0 if aligned.
        """
        # Get transforms and locations
        ref_transform = ref_car.get_transform()
        npc_transform = npc.get_transform()

        # Project relative vector onto the forward vector to get signed distance
        relative_distance = self._get_signed_distance(ref_transform, npc_transform)

        return relative_distance

    def _update_plan(self, pos_ahead):
        ref_car_loc = CarlaDataProvider.get_transform(self._ref_actor).location
        ref_car_wp = CarlaDataProvider.get_map().get_waypoint(
            ref_car_loc
        )
        wp_lists = ref_car_wp.next(pos_ahead)
        if wp_lists is None or len(wp_lists) == 0:
            raise RuntimeError("the road is not long enough")
        end_wp = wp_lists[0]
        if self._lane_opt == 'right_of':
            dest_wp = end_wp.get_right_lane()
        elif self._lane_opt == 'left_of':
            dest_wp = end_wp.get_left_lane()
        else:
            raise RuntimeError(f'Not supported lane option: {self._lane_opt}')
        ...
        self._plan = grp.trace_route(current_loc, dest_loc)
    ...
    def update(self):
        ...
        ref_speed = CarlaDataProvider.get_velocity(self._ref_actor)
        # print(f'ref_speed {ref_speed}')
        self._target_speed = max(self._target_speed, ref_speed * 3.6 * 2)  # speed hint, refine later
        # print(f'set target_speed to {self._target_speed}')
        rel_distance = self._get_relative_distance(self._ref_actor, self._actor)
        # print(f'------------log: {rel_distance} {self._rel_pos}------------')

        # todo: currently wait for timeout, if return SUCCESS, will apply ActorTransformSetter, need to refine later
        # if math.isclose(rel_distance, self._rel_pos, abs_tol=2.0):
        #     print('true!!!!!!!!!!!!!!!!!!!')
        #     new_status = py_trees.common.Status.SUCCESS
        #     return new_status

        if rel_distance < self._rel_pos and (not isinstance(self._actor, carla.Walker)): # have to adjust plan
            pos_ahead = self._rel_pos - rel_distance
            plan_ahead = -1
            if self._plan:
                end_wp = self._plan[-1][0]
                ref_transform = self._ref_actor.get_transform()
                plan_ahead = self._get_signed_distance(ref_transform, end_wp.transform)
            if plan_ahead < pos_ahead:
                guess_distance = 20
                self._update_plan(pos_ahead + guess_distance)
                self._update_local_planner()
        ...
        if self._actor is not None and self._actor.is_alive and self._local_planner is not None:
            ...
            if not isinstance(self._actor, carla.Walker):
                ...
                if (
                    (self._avoid_collision and detect_lane_obstacle(self._actor)) or
                    (rel_distance > self._rel_pos) # just wait
                ):
            ...
            # The walker is sent to the next waypoint in its plan
                ...
                if self._plan:
                    ...
                    control = self._actor.get_control()
                    control.speed = self._target_speed
                    control.direction = CarlaDataProvider.get_transform(self._actor).rotation.get_forward_vector()
                    self._actor.apply_control(control)
    ...
    def terminate(self, new_status):
        ...
        """
        if self._actor is not None and self._actor.is_alive:
            control, _ = get_actor_control(self._actor)
            self._actor.apply_control(control)
            if self._local_planner is not None and self._local_planner != "Walker":
                self._local_planner.reset_vehicle()
                self._local_planner = None

        super(WaypointFollower2, self).terminate(new_status)
