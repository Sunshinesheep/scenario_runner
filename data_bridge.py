...
class DataBridge(object):
    ...
    def __init__(self, world):
        ...
        self.other_actors = []
        self.other_actors_previous_location = []
        self.trace_list = []
        self._list_traffic_lights = []
        self._last_red_light = None
        
        print("Start writing trace.")
    ...
    def set_actors(self, ego_vehicle, other_actors):
        self.ego_vehicle = ego_vehicle
        self.other_actors = other_actors

    def update_ego_vehicle_start(self) -> None:
        """
    ...
    def update_npc_vehicle_start(self) -> None:
        for idx in range(len(self.other_actors)):
            npc = self.other_actors[idx]
            npc_id = "npc" + str(idx)
            npc_location = npc.get_location()
            npc_wp = self._map.get_waypoint(npc_location)
            
            self.trace['npcList'].append({
                "ID": npc_id,
                "name": npc.type_id,
                "groundTruthPerception": True,
                ...
                "start": {
                    "lane_position": {
                        "lane": "lane_" + str(npc_wp.road_id),
                        "offset": npc_wp.s,
                        "roadID": npc_wp.lane_id
                    },
                    "heading": {
                        "ref_lane_point": {
                            "lane": "lane_" + str(npc_wp.road_id),
                    ...
                    "speed": calculate_velocity(npc)
                }
            })
            
            self.trace["AgentNames"].append(npc_id)

    def update_npc_vehicle_motion(self) -> None:
        for idx in range(len(self.other_actors)):
            npc = self.other_actors[idx]
            npc_location = npc.get_location()
            npc_wp = self._map.get_waypoint(npc_location)
            ...
            if not 'motion' in self.trace['npcList'][idx]:
                self.trace['npcList'][idx]['motion'] = []
            
            self.trace['npcList'][idx]['motion'].append({
                ...
                "speed": calculate_velocity(npc),
                "brake": npc.get_control().brake,
                "linearAcceleration": {
                    "x": npc.get_acceleration().x,
                    "y": npc.get_acceleration().y,
                    "z": npc.get_acceleration().z
                }
            })

    def end_trace(self, path, scenario) -> None:
        """
        ...
        self.trace['ego']['destination'] = {
                                ...
                                "speed": calculate_velocity(self.ego_vehicle)
                            }
        
        for idx in range(len(self.other_actors)):
            npc = self.other_actors[idx]
            npc_wp = self._map.get_waypoint(npc.get_location())
            self.trace['npcList'][idx]['destination'] = {
                "lane_position": {
                    "lane": "lane_" + str(npc_wp.road_id),
                    "offset": npc_wp.s,
                    "roadID": npc_wp.lane_id
        ...
        fileidx = 0
        while os.path.exists(path + scenario +'/{}_{}_{}.json'.format(filetype, scenario, fileidx)):
            fileidx += 1

        with open(path + scenario +'/{}_{}_{}.json'.format(filetype, scenario, fileidx), 'w', encoding='utf-8') as fw:
            json.dump(self.trace, fw, indent=4)
    ...
    def update_trace(self) -> None:
        ...
        frame_dict['ego'] = {
            ...
            "isTrafficJam": self.isTrafficJam(),
            "isOverTaking": self.isOverTaking(),
            "isLaneChanging": self.isLaneChanging(),
            "isTurningAround": self.isTurningAround(turn=turn),
            "PriorityNPCAhead": self.PriorityNPCAhead(),
            "PriorityPedsAhead": False,
            "upperLimit" : float(upper_limit)
        }
        

        nearestNPC = self.getNearestNPC()
        if not nearestNPC is None:
            nearestNPC = nearestNPC.id
        
        NPCAhead = self.getNPCAhead()
        if not NPCAhead is None:
            NPCAhead = NPCAhead.id
        
        frame_dict['truth'] = {
            "obsList": self.getobsList(),
            "NearestNPC": nearestNPC,
            "minDistToEgo": self.getminDistToEgo(),
            "nearestGtObs": None,
            "NPCAhead": NPCAhead,
            "PedAhead": None,
            "NPCOpposite": None,
    ...
    def get_junction_ahead(self, ego_transform):
        ...
        distance_remain = 200
        current_wp = ego_wp
        while current_wp and current_wp.road_id == ego_wp.road_id and distance_remain > 0:
            waypoint_list.append(current_wp)
            current_wp = current_wp.next(1)[0]
            distance_remain -= 1
    ...
    def isTrafficJam(self):
        npc = self.getNPCAhead(driving_angle_limit = math.pi)
        if npc is None:
            return False
        
        # Speed restriction
        npc_speed = calculate_velocity(npc)
        return npc_speed < 0.5

    def isOverTaking(self, distance_limit = 20.0, direction_limit = math.pi / 6.0):
        if not self.isLaneChanging():
            return False
        ...
        for vehicle in self._world.get_actors().filter('vehicle.*'):
            if vehicle.id == self.ego_vehicle.id:
                continue
            # Speed restriction
            ego_speed = calculate_velocity(self.ego_vehicle)
            npc_speed = calculate_velocity(vehicle)
            # Distance restriction
            distance = self.ego_vehicle.get_location().distance(vehicle.get_location())
            # Direction restriction
            ego_forward_direction = self.ego_vehicle.get_transform().get_forward_vector()
            npc_forward_direction = vehicle.get_transform().get_forward_vector()
            driving_angle = ego_forward_direction.get_vector_angle(npc_forward_direction)
            if ego_speed > npc_speed and distance <= distance_limit and abs(driving_angle) <= direction_limit:
                return True
        return False

    def isLaneChanging(self):
        ego_transform = self.ego_vehicle.get_transform()
        road_wp = self._map.get_waypoint(ego_transform.location, project_to_road=True)
        distance_to_lane = ego_transform.location.distance(road_wp.transform.location)

        ego_length = self.ego_vehicle.bounding_box.extent.x
        ego_width = self.ego_vehicle.bounding_box.extent.y
        ego_theta = math.radians(self.ego_vehicle.bounding_box.rotation.yaw - road_wp.transform.rotation.yaw)

        return distance_to_lane + ego_length*abs(math.sin(ego_theta)) + ego_width*abs(math.cos(ego_theta)) > road_wp.lane_width

    def isTurningAround(self, turn):
        ego_transform = self.ego_vehicle.get_transform()
        ego_wp = self._map.get_waypoint(ego_transform.location)
        return ego_wp.is_junction and turn != 0 and self.isLaneChanging()

    def PriorityNPCAhead(self, location_angle_limit_1 = math.pi * 3.0 / 4.0, location_angle_limit_2 = math.pi / 2.0):
        DIST_RANGE = 16.0
        ego_transform = self.ego_vehicle.get_transform()
        ego_wp = self._map.get_waypoint(ego_transform.location)
        ...
        for vehicle in self._world.get_actors().filter('vehicle.*'):
            ...
            if distance <= DIST_RANGE:
                ego_forward_direction = self.ego_vehicle.get_transform().get_forward_vector()
                ego2npc_location = vehicle.get_location() - self.ego_vehicle.get_location()
                ego2npc_direction = carla.Vector3D(x = ego2npc_location.x, y = ego2npc_location.y, z = ego2npc_location.z)
                location_angle = ego_forward_direction.get_vector_angle(ego2npc_direction)
                if (ego_wp.is_junction and abs(location_angle) <= location_angle_limit_1) or abs(location_angle) <= location_angle_limit_2:
                    return True
    ...
    def getobsList(self):
        ...
        for vehicle in self._world.get_actors().filter('vehicle.*'):
            ...
            distToEgo = self.ego_vehicle.get_location().distance(vehicle.get_location())
            npc_speed = calculate_velocity(vehicle)
            obsList.append({"name":vehicle_id, "distToEgo":distToEgo, "speed":npc_speed})
        
        return obsList

    def getminDistToEgo(self):
        minDistToEgo = 200.0
        for vehicle in self._world.get_actors().filter('vehicle.*'):
            if vehicle.id == self.ego_vehicle.id:
                continue
            distance = self.ego_vehicle.get_location().distance(vehicle.get_location())
            if distance < minDistToEgo:
                minDistToEgo = distance
        return minDistToEgo

    def getNearestNPC(self):
        DIST_RANGE = 16.0
        nearest_npc = None
        min_dist = DIST_RANGE
    ...
    def getNPCAhead(self, location_angle_limit = math.pi / 6.0, driving_angle_limit = math.pi / 3.0):
        DIST_RANGE = 16.0
        npc_ahead = None
        min_dist = DIST_RANGE
        ...
        for vehicle in self._world.get_actors().filter('vehicle.*'):
            ...
            if distance <= min_dist:
                ...
                npc_forward_direction = vehicle.get_transform().get_forward_vector()
                driving_angle = ego_forward_direction.get_vector_angle(npc_forward_direction)
                if abs(location_angle) <= location_angle_limit and abs(driving_angle) <= driving_angle_limit:
                    min_dist = distance
                    npc_ahead = vehicle
